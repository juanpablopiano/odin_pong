...
update :: proc(g: ^Game, i: Input, dt: f32) {
	...
	if rl.CheckCollisionCircleRec(b.pos, b.radius, rl.Rectangle{p1.pos.x, p1.pos.y, p1.size.x, p1.size.y}) {
		b.speed.x *= -1
	}
	if rl.CheckCollisionCircleRec(b.pos, b.radius, rl.Rectangle{p2.pos.x, p2.pos.y, p2.size.x, p2.size.y}) {
		b.speed.x *= -1
	}
}
...
reset_ball :: proc(b: ^Ball) {
	speed : f32 = 500
	b.pos = {WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2}
	b.speed = {
...
draw :: proc(g: Game) {
	...
	rl.DrawLineDashed({WINDOW_WIDTH/2, 0}, {WINDOW_WIDTH/2, WINDOW_HEIGHT}, 20, 20, rl.WHITE)
	...
	rl.DrawText(fmt.ctprint(g.score.x), WINDOW_WIDTH / 4, 50, 120, rl.WHITE)
	rl.DrawText(fmt.ctprint(g.score.y), WINDOW_WIDTH * 3 / 4, 50, 120, rl.WHITE)
}
